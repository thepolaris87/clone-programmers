...
export const postSignUp = async (body: { name: string; email: string; password: string }) => {
    ...
    return axios.post('http://192.168.2.41:9000/coms/sign-up', body).then((r) => {
        console.log(r);
        return r.data;
    });
};
...
export const postSignIn = async (body: { email: string; password: string }) => {
    return axios.post('http://192.168.2.41:9000/coms/sign-in', body).then((r) => r.data);
};

export const getQuestion = async (questionId: string) => {
    return axios.get(`http://192.168.2.41:9000/learn/courses/${questionId}`).then((r) => r.data);
};

export const postQuestion = async (body: { questionId: string; userCode: string; status: string }) => {
    return axios.post(`http://192.168.2.41:9000/learn/courses/${body.questionId}`, { userCode: body.userCode, status: body.status }).then((r) => r.data);
};
