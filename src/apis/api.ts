...
// const client = axios.create({ baseURL: 'http://192.168.3.112:9000' });
...
export const postSignUp = async (body: { name: string; email: string; password: string }) => {
    ...
    return axios.post('http://192.168.3.112:9000/coms/sign-up', body).then((r) => {
        return r.data;
    });
};

export const postSignIn = async (body: { email: string; password: string }) => {
    return axios.post('http://192.168.3.112:9000/coms/sign-in', body).then((r) => r.data);
};
